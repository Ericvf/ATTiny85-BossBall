...
void Adafruit_NeoPixel::All(uint32_t c) {
  for (int i = 0; i < numLEDs; i++) { // Arrays are 0-indexed, thus NOT '<='
    setPixelColor(i, c);
  }
}

void Adafruit_NeoPixel::Show(uint32_t _delay) {
	show();
	delay(_delay);
}


void Adafruit_NeoPixel::Set(uint16_t n, uint32_t c) {
	setPixelColor(n, c);
}

uint32_t Adafruit_NeoPixel::Rgb(uint8_t r, uint8_t g, uint8_t b) {

	/*	uint32_t c =  ((long)r & 0xFF)<<16 | ((long)b & 0xFF)<<8 | (long)g & 0xFF;*/
